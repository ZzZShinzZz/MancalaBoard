...
/**
 * A GameFrame class that acts as VIEW and CONTROLLER in MVC Architecture.
 */
public class GameFrame extends JFrame implements ChangeListener, MouseListener {
	...
	private GameModel dataModel;
	private int[] stoneNumX, stoneX, pressX;
	private int aNumY, aStoneY, aPressY;
	private int bNumY, bStoneY, bPressY;
	private int wid, len;
	private Font countf = new Font("Helvetica",Font.BOLD,18);
	private Font turnf = new Font("Helvetica",Font.BOLD,22);
	...
	/**
	 * These methods below are just dummy methods
	 * @param e
	 */
	...
	/**
	 * Implement mousePress to get the current position of which PIT that the user selects so that we can distribute the stone from that pit
	 * @param e
	 */
	...
	/**
	 * 
	 * @param event - updating the UI based the which event is being fired.
	 * precondition: 
	 * postcondition: 
	 */
	...
	/**
	 * 
	 * @param g -Use to draw (paint) the current state of BOARD and STONE which should be called each time the BOARD/STONE has changed
	 * precondition: 
	 * postcondition: 
	 */
	...
	private class MenuFrame extends JFrame  {
		...
		/**
		 * Constructs the menu frame that takes the GameFrame (main window) as parameter to control the flow of interaction between 2 windows
		 * @param gf : a GameFrame object so that we can ensure interaction between GameFrame and MenuFrame is flawless.
		 * precondition: 
		 * postcondition: 
		 */
		...
		/**
		 * Set up the menu window which initialize JComponents
		 */
		private void setupUI() {
			...
			try {
				// RESIZING THE INITIAL IMAGE OF THE BOARD A
				...
				// RESIZING THE INITIAL IMAGE OF THE BOARD B
